...
2. 第一版本内置阿里云百炼 `qwen3.6-flash-2026-04-16` 配置。用户可以填写和保存 API Key，也可以修改模型名称。系统提供测试连接入口并安全保存 API Key。
3. 普通对话只使用当前已配置的云端模型提供方；缺少 API Key 时，系统必须提示用户先完成模型设置。
4. 连接测试必须向当前模型提供方发起最小请求，验证接口、API Key 和模型名称是否可用。
