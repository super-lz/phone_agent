...
9. OpenAI 兼容接入方使用 Chat Completions 协议；Anthropic Claude 使用 Messages API 协议；协议差异不能通过错误伪装成同一种请求格式。
...
- 聊天界面必须以圆环形式展示当前会话上下文用量；用量接近或超过预算时，用户应能看到是否已使用保守预算、已压缩早期对话，以及主要上下文来源占用。
