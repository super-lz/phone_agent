...
- 第一版本不实现语音输入或输出、不实现远程终端、不建设云端运行时、不做后台长期自主执行或定时自主任务、不做插件市场、不允许任意网页默认调用手机能力、不实现 iOS 本机 shell、不实现完整 Office 编辑器。
- 用户已经明确发起且正在运行的 Agent 会话不属于后台自主任务；系统应在平台允许范围内继续处理或恢复处理，避免用户切到后台后丢失当前请求。
- 终端是未来高级后备能力；如果手机本地 Capability 足够强，第一版本可以完全不实现终端。
- 远端运行时只保留抽象入口，不作为第一版本验收条件。
- 第一版本默认使用阿里云百炼 OpenAI 兼容接口接入 `qwen3.6-flash-2026-04-16` 进行模型联调；模型设置页必须支持多个国内外主流模型接入方，每个接入方独立保存 API Key 和模型选择，并允许用户在内置模型列表之外输入自定义模型名称；其它上下文和生成参数使用推荐默认值。
...
4. 工具路由模型失败、超时或输出不可解析时，系统仍必须对用户最新消息应用本地确定性兜底规则；明显的长期记忆写入、笔记记录、工作区创建或切换、联网查询、报告、文档、任务清单等可复用产物创建、Web App 创建、手机能力调用等请求不得因为路由模型不可用而退化成普通聊天。
...
8. 用户必须能停止当前 Agent 运行；停止后输入区恢复可用，系统不能让一次卡住的模型流长期阻塞后续操作；本轮可恢复记录和前台服务必须随停止请求清理，已经展示到对话中的进行中过程必须持久化为已停止状态，应用刷新或重启后不得继续显示为“正在思考”或自动恢复这轮已停止任务。
9. Capability Runtime 校验工具输入、权限策略和执行边界后调用对应 adapter。
10. 高风险能力需要用户确认时，权限请求必须作为可恢复任务状态持久化，至少保留请求 ID、工具名、Capability、Workspace、参数摘要和用户原始请求；用户稍后或应用重启后批准或拒绝时，系统必须基于该原始请求和工具结果继续完成同一个任务，而不是只执行孤立工具或丢失任务目标。
11. 工具结果以结构化结果返回给 Agent，并以工具轨迹展示给用户；同一次用户请求中的中间模型轮次、工具调用和工具结果默认应收敛到同一个回复气泡的折叠执行过程里，最终回答和可复用产物卡片保持外露；工具调用卡片应优先展示工具名称、目标文件、Artifact、URL、权限或其它关键参数摘要，避免直接展开大段生成内容、源码、HTML、CSS、JS 或原始 JSON；模型准备工具调用前已经输出的临时说明或代码也应归入折叠过程，不能在生成中用大段源码淹没会话；联网搜索、网页解析和手机本地能力结果必须优先展示为用户可理解的摘要或结果卡片，原始结构化数据只能作为调试详情折叠展示。
12. Agent 基于工具结果继续回答；失败时说明原因并给出替代方案。
13. 模型流式连接必须有接收空闲超时；如果连接已经建立但长时间没有新 token、工具调用或完成事件，系统必须返回可诊断错误并恢复输入，而不是无限保持发送中。
14. 单次用户请求的 Agent Loop 默认由模型决定何时完成：模型返回最终自然语言回答时结束；系统不应把固定模型轮次、总工具调用次数或连续无进展次数作为正常完成条件。部署方可以显式配置这些可选的失控保护边界。
...
17. 当用户明确要求联网搜索、查询最新信息、提供来源，或读取具体网页 URL 时，对应联网工具调用是必需动作；在搜索或网页读取成功前，Agent 不得基于模型内置知识声称已经完成最新信息查询或网页读取。
...
1. 用户可以进入模型设置页配置云端模型提供方。
2. 模型设置页按国内模型、国外模型和聚合平台组织接入方，用户应能快速看到每个接入方的默认模型、协议类型、接口文档入口和 API Key 获取入口。
3. 第一版本内置接入方包括阿里云百炼、DeepSeek、Kimi/Moonshot、智谱 GLM、腾讯混元、MiniMax、小米 MiMo、硅基流动、OpenAI、Google Gemini、xAI、Mistral、Anthropic Claude 和 OpenRouter。
4. 每个接入方必须独立保存 API Key 和模型选择。切换接入方时，系统加载该接入方自己的 API Key 和模型配置，不能用一个全局 Key 或模型名覆盖所有接入方。
5. 每个接入方必须提供默认模型名称列表，用户默认通过选择器选择模型；当内置列表不满足需求时，用户可以输入自定义模型名称，自定义值也按接入方独立保存。
6. 模型设置页必须展示当前模型的最大上下文窗口；内置模型使用已配置的模型级窗口，未知或自定义模型必须明确标识窗口未知，并允许用户手动覆盖最大上下文 token 数。
7. 普通对话只使用当前已配置且已启用调用的云端模型提供方；缺少 API Key 时，系统必须提示用户先完成模型设置。
8. 连接测试必须向当前模型提供方发起最小请求，验证接口、API Key 和模型名称是否可用；如果某接入方的官方 API endpoint 尚未确认，系统可以允许保存配置，但必须明确标识暂不可测试和暂不可用于普通对话。
9. 连接测试的 UI 结论必须来自 HTTP 状态、接口错误和是否返回可解析模型响应；成功只展示“连接成功”和被测试的真实模型，失败只展示失败原因。模型生成的自然语言正文不能被当作测试结论拼进成功或失败文案。
10. OpenAI 兼容接入方使用 Chat Completions 协议；Anthropic Claude 使用 Messages API 协议；协议差异不能通过错误伪装成同一种请求格式。
...
16. Web App 创建或更新工具在写入项目文件、manifest 和版本记录后，必须自动执行同一项目的受控静态测试，并在工具输出中返回测试结果；测试未通过时不得把项目描述为完全完成。
...
- 对话中用于展示任务过程的临时中间文本可以持久化为消息轨迹，但不得作为后续模型的普通对话语义历史，也不得计入近期历史上下文预算；后续上下文应保留用户消息、最终回答、工具结果摘要、Artifact 和错误结论，避免“正在分析”“开始调用工具”等过程状态污染下一轮判断或上下文用量估算。
- 单条历史消息如果已经超过本轮近期上下文预算，不得为了保留至少一条 recent 原文而强行塞入模型请求；应把它纳入压缩摘要，避免旧的大段输出阻断下一轮任务。
- 会话上下文压缩必须按当前模型上下文窗口、预留输出空间、系统提示、工具 schema、长期记忆、附件摘要、本轮输入和近期历史的估算占用动态触发；模型窗口未知时使用保守预算，并在 UI 中明确标识该预算不是官方精确值。
- 聊天界面必须以完整浅色轨道和前景进度弧组成的圆环展示最近一次已知会话上下文用量；新一轮尚未重新计算完成时继续显示上一轮用量，应用重启后也应恢复当前 Workspace 的最近一次用量。只有确实没有历史用量时才显示轻量待计算状态，不得显示 `--`、`0%` 等调试或误导性占位符；用量接近或超过预算时，用户应能看到是否已使用保守预算、已压缩早期对话，以及主要上下文来源占用。
- 每次完成实际模型调用的 Agent 运行都应记录本机估算 Token 用量，包括输入估算、预留输出、模型、提供方、运行时长和发生日期；该统计用于用户理解本机使用趋势，不等同于模型提供方账单中的精确 usage。
- 用户必须能进入独立 Token 用量页面查看累计用量、单次峰值、最长任务时长、连续使用天数、按日/周/累计切换的活动热力图，以及模型和能力使用洞察。
...
- 用户必须能手动清理本地工作区数据；清理范围包括 Workspace、会话、长期记忆、Note、Artifact、Web App、本地工作区文件、工具审计记录和本机 Token 用量统计，但不得删除模型名称、模型提供方设置或 API Key。
...
- 文件、数据库、记忆、Workspace、Artifact、Office/PDF 文档、时间、定位、剪贴板、相机、媒体选择、系统文件选择、麦克风录音、联系人选择、二维码/条码识别、通知、日历、应用信息、设备信息、屏幕指标读取、屏幕常亮与方向控制、系统音量控制、系统 UI 显隐控制、WebView、Skill 和 MCP 的 Capability 定义。
- Artifact 中心。
- 全局长期记忆和会话上下文。
...
- Agent Loop 必须支持可选的任务预算，并在日志中暴露当前工具调用消耗；默认不设置模型轮次、工具调用次数、连续失败或无进展上限，避免用固定阈值代替模型的完成判断。
...
- 每个本地 Web App 项目必须拥有独立文件目录；即使模型只提供 `index.html`、`styles.css`、`app.js` 等根路径文件，系统也必须自动套入独立项目目录，避免多个 Web App 覆盖同一入口或资源文件。
- 维护、复测、版本和回滚本地 Web App 时，系统必须优先使用 Web App Artifact ID；当模型只提供同一 Workspace 内唯一匹配的项目 ID、入口路径、项目目录名或系统自动生成目录的短名时，也应能解析到原 Artifact。存在多个候选时必须返回结构化歧义错误，不能自动猜测目标项目。
...
- 当用户询问屏幕尺寸、分辨率、像素比、安全区、键盘 inset、亮暗模式、语言环境、文字缩放，或本地 Web App/演示需要根据当前显示环境适配布局时，Agent 可以调用 `screen.metrics`；结果必须包含逻辑尺寸、物理尺寸、设备像素比、方向、安全区/视图边距、平台亮暗模式、locale、文字缩放和可访问性摘要。该能力只读取当前 Flutter View/平台显示指标，不承诺读取其它 App 的窗口或截屏内容。
...
- 用户能从应用内清理本地工作区数据；清理后只保留默认 Workspace，工作区会话、文件、Note、Artifact、Web App、长期记忆、工具审计记录和本机 Token 用量统计被清空，模型名称和 API Key 仍保留。
- 用户完成模型对话后，能从 Token 用量页面看到本机估算的历史 Token 用量、活动热力图、连续使用天数、任务时长和常用模型/能力排行；页面必须清楚表达该数据是本机估算，不承诺等同于模型提供方账单。
...
- 聊天输入区上方必须有常驻运行信息条：左侧展示最近一次已知上下文用量，右侧在空闲时展示可一键填入输入框的推荐对话内容，在 Agent 运行中展示当前阶段、具体步骤和当前工具。工具调用预算、最大次数等内部控制数字只应进入日志或诊断信息，不应作为常驻用户界面文案。停止入口应收敛在输入区发送按钮位置，发送中由“发送”切换为“停止”，不在输入框上方重复展示停止按钮。模型流、工具执行或前后台恢复阶段卡住时，用户应能从 UI 和日志判断当前卡在哪一步。
...
- AI 在复杂任务中能连续完成任意必要轮次的工具调用；除非用户取消、权限等待、上下文/传输边界、显式配置的保护边界或模型自然结束，系统不得过早停止工具链。
...
- Web App 预览内容必须铺满预览页可用视口，右侧和底部不得露出宿主 Flutter 页面背景或默认 WebView 空白边。
...
- Web App 创建或更新后，工具输出必须包含受控静态测试结果；测试结果应返回是否通过、检查文件和问题列表，明显的入口缺失、引用缺失或 HTML/CSS/JS 结构问题不得被当作成功忽略。
...
- 模型流式生成的工具参数如果最终不是有效 JSON，系统必须向 Agent 返回结构化 `invalid_tool_arguments` 结果并要求重新生成完整参数，不能把半截参数传入真实 Capability 执行。
...
- 模型流式连接因应用切到后台、网络切换或系统关闭连接而中断时，系统必须展示可读错误；如果尚未收到任何模型正文，可自动重试一次；如果本轮只收到尚未执行的工具参数增量，重试前必须丢弃半截参数并重新发起模型请求，避免把残缺 JSON 当作真实工具调用；如果重试发生时应用仍在后台，应等应用回到前台后再重试；一旦已收到正文内容或已经开始执行真实 Capability，不得自动重放同一轮模型请求，避免重复工具执行或重复内容。
- 搜索服务不可达、搜索结果解析失败、网页读取超时、目标 URL 无效或目标网页不可读时，联网能力必须返回结构化错误，并允许 Agent 解释限制或尝试替代方案。
- 本地持久化中的单个消息记录或消息内容块损坏时，不得导致整个 Workspace 会话不可读；系统应保留可恢复的消息内容，并用可见错误块标识无法恢复的部分。
