...
6. 普通对话只使用当前已配置且已启用调用的云端模型提供方；缺少 API Key 时，系统必须提示用户先完成模型设置。
7. 连接测试必须向当前模型提供方发起最小请求，验证接口、API Key 和模型名称是否可用；如果某接入方的官方 API endpoint 尚未确认，系统可以允许保存配置，但必须明确标识暂不可测试和暂不可用于普通对话。
8. OpenAI 兼容接入方使用 Chat Completions 协议；Anthropic Claude 使用 Messages API 协议；协议差异不能通过错误伪装成同一种请求格式。
